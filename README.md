...
- **以小時計時數採固定上班時段公式**：全站固定上班時段 09:00–18:00、午休 12:00–13:00 不計入時數；同日與跨日皆將起訖時間夾在上班時段內並扣除與午休重疊的時間，跨日另加中間完整天數 × 8 小時，結果四捨五入至 0.5 小時。中間天數一律以完整工作日（8 小時）計算，不排除週末與國定假日，與天數計算的取捨一致。換算後不足 0.5 小時（即為 0）的申請會在表單驗證擋下，避免送出後在清單顯示為「0 小時」的請假紀錄。
